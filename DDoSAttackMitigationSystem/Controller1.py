from operator import attrgetter # After f = attrgetter('name'), the call f(b) returns b.name.
...
# python自带
import socket # 17.2. socket — Low-level networking interface
import threading # 16.2. threading — Higher-level threading interface
import SocketServer # 20.17. SocketServer — A framework for network 
import subprocess # 17.1. subprocess — Subprocess management
import logging # 15.6. logging — Logging facility for Python
...
class Client:
    ...
    def send(self, message):
        def do():
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # These constants represent the address (and protocol) families, used for the first argument to socket(). These constants represent the socket types, used for the second argument to socket(). 
        ...
        thread = threading.Thread(target=do) #A class that represents a thread of control. This class can be safely subclassed in a limited fashion.
        thread.daemon = True
        thread.start()
